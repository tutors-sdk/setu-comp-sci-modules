...
This script generates the `tutors` folder structure from the `module_catalogue` source data.
...
The `module_catalogue/` directory contains:
...
└── .catalogue-order.yaml  # Display order for programmes and clusters (project root)
```

## Output Structure

The `tutors/` directory is generated with:
...
- `properties.yaml` - Course metadata (copied from `tutors-reference/` if available)
- `course.md` - Course overview text (copied from `tutors-reference/` if available)
- `course.png` - Course banner image (copied from `tutors-reference/` if available)

If these files don't exist in `tutors-reference/`, defaults are created.

All other content is regenerated from the source data.

**Note:** The `tutors-reference/` directory contains the original reference implementation used for extracting icons, images, and default course files.
...
module_catalogue/
    programmes/yaml/ ─────┐
    schedules/yaml/  ─────┤
    modules/yaml/    ─────┼──→ CatalogueGenerator ──→ tutors/
...
Ensure the PDF exists in `module_catalogue/descriptors/pdf/<CODE>.pdf` with the correct module code.
